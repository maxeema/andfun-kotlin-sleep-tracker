...
class JournalAdapterDecoration(@ColorRes color: Int) : RecyclerView.ItemDecoration() {
    ...
    private fun draw(c: Canvas, r: RecyclerView) {
        ...
        if (r.clipToPadding) {
            ...
            start = SKIPPED_HORIZONTAL_OFFSET
        ...
        val childHeight = r.getChildAt(JournalAdapter.FIRST_REAL_ITEM_INDEX)?.height?.takeIf { it > 0 } ?: return
        while (true) {
            bottom += childHeight
            if (bottom + IGNORED_BOTTOM_REMINDER >= r.height)
                return
    ...
    override fun getItemOffsets(rect: Rect, view: View, recycler: RecyclerView, st: RecyclerView.State) {
        if (view.tag == JournalAdapter.virtualFooter) {
            rect.set(0, 0, 0, recycler.getChildAt(JournalAdapter.FIRST_REAL_ITEM_INDEX).height)
        } else {
            rect.set(0, 0, 0, LINE_SIZE)
        }
    }
